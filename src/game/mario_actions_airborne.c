...
void play_knockback_sound(UNUSED struct MarioState *m) {
    // unused
}
...
void update_flying_yaw(struct MarioState *m) {
    ...
    if (m->angleVel[2] > 0x2000) {
        m->angleVel[2] = 0x2000;
    } else if (m->angleVel[2] < -0x2000) {
        m->angleVel[2] = -0x2000;
    }
...
void update_flying_roll(struct MarioState *m) {
    if (m->flags & MARIO_WING_CAP) {
        (m->forwardVel > 112.0f)
        ? (m->faceAngle[2] += 64.0f * (m->forwardVel / 3.0f))
...
s32 act_wall_slide (struct MarioState *m) {
    if (gKickTimer > 0 && m->wallKickTimer == 0 && m->prevAction == ACT_AIR_HIT_WALL) {
        ...
        if (!gKickToggle) {
            ...
            if (m->wallKickTimer == gKickTimer) m->vel[1] = 0;
        }
    ...
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_LANDED:
            if (m->floor->type == SURFACE_BURNING) {
                ...
                m->hurtCounter += (m->flags & MARIO_CAP_ON_HEAD) ? 12 : 18;
                (!g95Toggle) ? (m->vel[1] = 80.0f) : (m->vel[1] = 64.0f);
                play_sound(SOUND_MARIO_ON_FIRE, m->marioObj->header.gfx.cameraToObject);
#if ENABLE_RUMBLE
                queue_rumble_data(5, 80);
#endif
