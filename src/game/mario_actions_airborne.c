...
s32 check_fall_damage(struct MarioState *m, u32 hardFallAction) {
    ...
    if (m->flags & MARIO_METAL_CAP) {
        if (!(gMovesetVar & REAL)) return FALSE;
		m->damage = -0x100;
    ...
    if (m->vel[1] < -mTerminalVel && mTerminalVel != 0 && fallHeight > damageHeight && m->floor->type != SURFACE_BURNING) {
		...
		u16 multiplier = SLICE40 * ((m->marioBodyState->wingFlutter == FALSE) ? 6 : 3);
		m->damage = MIN(m->vel[1] + mTerminalVel, 0) * multiplier;
		if (!(gMovesetVar & REAL)) m->squishTimer = ((-m->damage < SLICE * 4) ? 30 : 60);
...
s32 auto_dive(struct MarioState *m) {
    ...
    if (m->input & INPUT_B_PRESSED) {
        ...
        if ((gMovesetVar & 0b11) != 2) return set_mario_action(m, ACT_JUMP_KICK, 0);
...
void update_air(struct MarioState *m) {
    if (!check_horizontal_wind(m)) {
        ...
        if (m->input & INPUT_NONZERO_ANALOG) {
            ...
            f32 turnSpd = (!M_LUIGI) ? 16.0f : 12.0f;
        ...
        /*
        m->slideVelX += sideVel * sins(m->faceAngle[1] + 0x4000);
        m->slideVelZ += sideVel * coss(m->faceAngle[1] + 0x4000);
        */
...
void update_lava_boost_or_twirling(struct MarioState *m) {
    ...
    if (m->input & INPUT_NONZERO_ANALOG) {
        ...
        if (m->action == ACT_TWIRLING) m->forwardVel += 0.375f - (twirlMulti * intendedMag);
    }
...
u32 common_air_action_step(struct MarioState *m, u32 landAction, s32 animation, u32 stepArg) {
    ...
    switch (stepResult) {
        case AIR_STEP_NONE:
            if (M_LUIGI && aGravToggle && m->input & INPUT_A_DOWN && m->vel[1] <= 0) {
...
s32 act_jump(struct MarioState *m) {
    if (gMovesetVar & A_MASK) {
        if (m->flags & MARIO_WING_CAP && (gMovesetVar & (~A_HARD & A_MASK))) {
...
s32 act_backflip(struct MarioState *m) {
    ...
    if ((gMovesetVar & 3) != C_LUIGI) common_air_action_step(m, ACT_BACKFLIP_LAND, MARIO_ANIM_BACKFLIP, 0);
...
s32 act_freefall(struct MarioState *m) {
    ...
    switch (m->actionArg) {
        ...
        case ACT_ARG_FREEFALL_FROM_SLIDE_KICK:
            ...
            break;
        case 4: // coyote time
            if (gMovesetVar & (A_HARD | REAL)) m->actionArg = 0;
            m->vel[1] += 3.0f;
    ...
    common_air_action_step(m, ACT_FREEFALL_LAND, animation, AIR_STEP_CHECK_LEDGE_GRAB | AIR_STEP_CHECK_HANG);
    return FALSE;
}

s32 act_hold_jump(struct MarioState *m) {
    if (gMovesetVar & A_HARD) {
        set_mario_action(m, m->prevAction, 0);
        return FALSE;
    }
    ...
    if (m->input & INPUT_B_PRESSED && m->prevAction != ACT_HOLD_BUTT_SLIDE) {
        if (m->heldObj->oInteractionSubtype & INT_SUBTYPE_HOLDABLE_NPC)
            return set_mario_action(m, ACT_FREEFALL, 0);
...
s32 act_side_flip(struct MarioState *m) {
    ...
    if (gMovesetVar & REAL) return set_mario_action(m, ACT_JUMP, 0);
...
s32 act_long_jump(struct MarioState *m) {
    ...
    common_air_action_step(m, ACT_LONG_JUMP_LAND, MARIO_ANIM_SLOW_LONGJUMP, AIR_STEP_CHECK_LEDGE_GRAB);
#if ENABLE_RUMBLE
...
s32 act_twirling(struct MarioState *m) {
    ...
    s16 yawVelTarget =
    ((twirlMulti >= 1.0f) ? twirlMulti : (1.0f / twirlMulti)) * (!(gMovesetVar & DEMO) || (gMovesetVar & REAL) ? 0x1400 : 0x1800);
...
s32 act_dive(struct MarioState *m) {
    if (m->input & INPUT_A_PRESSED && m->actionTimer > 0 && !(gMovesetVar & DEMO)) {
...
#endif
                play_sound(SOUND_MARIO_OOOF2, m->marioObj->header.gfx.cameraToObject);
                if (gGlobalTimer % 2 == 0) m->particleFlags |= PARTICLE_MIST_CIRCLE;
...
s32 act_ground_pound(struct MarioState *m) {
    ...
    if (m->input & (INPUT_A_DOWN | INPUT_B_DOWN | INPUT_Z_DOWN) && m->actionTimer > 8 && !(gMovesetVar & DEMO)) {
        m_spawn_dust(PARTICLE_DUST, JUMP);
        set_mario_action(m, ACT_SOFT_BONK, 0);
    }
    ...
    m->forwardVel = approach_f32_symmetric(m->forwardVel, 0, ABS(m->forwardVel / MAX(-m->actionTimer * 2 + 256, 2)));
    mario_set_forward_vel(m, m->forwardVel);

    u8 speed = 0x10;
    if (m->actionArg > 0 && m->actionTimer < 8) speed = -m->actionTimer + 0x18;
...
u32 common_air_knockback_step(struct MarioState *m, u32 landAction, u32 hardFallAction, s32 animation, f32 speed) {
...
#endif
            if (m->actionArg != 3 && gMovesetVar & REAL) {
                m->hurtCounter += 2;
                return set_mario_action(m, ACT_HARD_BACKWARD_GROUND_KB, 3);
            }
    ...
    if (!(gMovesetVar & REAL)) {
        if (m->actionTimer > 25) {
            ...
            if (!(gMovesetVar & DEMO)) set_mario_action(m, ACT_FREEFALL, 0);
...
f32 wallkickVel = 0.0f;

s32 check_wall_kick(struct MarioState *m) {
    if ((m->input & (INPUT_A_PRESSED | INPUT_B_PRESSED) && m->prevAction == ACT_AIR_HIT_WALL)) {
        m->forwardVel = wallkickVel;
        m->faceAngle[1] += 0x8000;
        if (gMovesetVar & A_MASK) return set_mario_action(m, ACT_FREEFALL, 0);
...
u32 wall_kick_step(struct MarioState *m, u32 landAction, s32 animation) {
    u32 stepResult = perform_air_step(m, 0);
    switch (stepResult) {
        case AIR_STEP_HIT_WALL:
            set_mario_animation(m, animation);
        ...
        case AIR_STEP_LANDED:
            ...
            break;
        case AIR_STEP_NONE:
            /* Enforce a minimum speed to guarantee landing on the ledge */
            mario_set_forward_vel(m, MAX(wallkickVel, m->vel[1]));
...
s32 act_wall_slide (struct MarioState *m) {
    if (gKickVar & (1 << 7)) {
        ...
        if (gGlobalTimer % 2 == 0) m->particleFlags |= PARTICLE_DUST;
        mario_set_forward_vel(m, 0.0f);
    } else if ((gMovesetVar & REAL) && m->wallKickTimer == (gKickVar & 0x7F)) m->vel[1] = 0;
    ...
    if (gKickVar & (1 << 7)) {
        ...
        wall_kick_step(m, ACT_FREEFALL_LAND, MARIO_ANIM_START_WALLKICK);
        m->marioObj->header.gfx.angle[1] += 0x8000; }
    else set_mario_animation(m, MARIO_ANIM_START_WALLKICK);

    if ((gKickVar & 0x7F) > 0 && m->wallKickTimer == 0 && m->prevAction == ACT_AIR_HIT_WALL) {
        play_sound(SOUND_MARIO_UH, m->marioObj->header.gfx.cameraToObject);
        if (gMovesetVar & REAL) return set_mario_action(m, ACT_BACKWARD_AIR_KB, 2);
...
s32 act_getting_blown(struct MarioState *m) {
    ...
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_LANDED:
            set_mario_action(m, ACT_HARD_BACKWARD_AIR_KB, 1);
            break;
...
s32 act_air_hit_wall(struct MarioState *m) {
    if (m->prevAction == ACT_FREEFALL) return set_mario_action(m, ACT_FREEFALL, 0); /* band-aid fix for short platforms / small overhangs */
    if (m->heldObj != NULL) { mario_drop_held_object(m); }

    m->wallKickTimer = gKickVar & 0x7F;
    wallkickVel = m->forwardVel;

    if (gKickVar & (1 << 7)) {
        wallkickVel += MAX(m->vel[1] / 2.0f, 0);
        if (m->vel[1] < 20.0f) m->vel[1] = 20.0f;
        m->vel[1] += (m->forwardVel / 8.0f);
        set_mario_action(m, ACT_WALL_SLIDE, 0);
    } else {
        if (!(gMovesetVar & REAL)) m->vel[1] = 24.0f;
        set_mario_action(m, ACT_WALL_SLIDE, 0);
    }
...
s32 act_lava_boost(struct MarioState *m) {
    ...
    if ((m->area->terrainType & TERRAIN_MASK) != TERRAIN_SNOW && !(m->flags & MARIO_METAL_CAP)
        && m->vel[1] > 0.0f) {
        set_camera_shake_from_hit(SHAKE_BURN);
        m->particleFlags |= PARTICLE_FIRE;
...
s32 act_slide_kick(struct MarioState *m) {
    ...
    if (gMovesetVar & DEMO) return set_mario_action(m, ACT_FREEFALL, 2);
    if (M_LUIGI) return set_mario_action(m, ACT_SHOT_FROM_CANNON, 0);
    ...
    switch (m->actionArg) {
        ...
        case 0:
            ...
            m->forwardVel = approach_f32(m->forwardVel, SKspeed, inc, inc);
            break;
        case 1:
...
s32 act_shot_from_cannon(struct MarioState *m) {
    ...
    if (m->actionArg == 0) {
        ...
        m->forwardVel = approach_f32_symmetric(m->forwardVel, 48.0f, (m->forwardVel - 48.0f) / 128.0f);
    }
    mario_set_forward_vel(m, m->forwardVel);
    ...
    switch (perform_air_step(m, 0)) {
        ...
        case AIR_STEP_LANDED:
            set_mario_action(m, ACT_DIVE_SLIDE, 0);
            m->faceAngle[0] = 0;
            if (m->actionArg != 0) set_camera_mode(m->area->camera, m->area->camera->defMode, 1);
#if ENABLE_RUMBLE
            queue_rumble_data(5, 80);
#endif
            break;
