...
struct LandingAction     sLongJumpLandAction = {         4,               5,      ACT_FREEFALL,     ACT_LONG_JUMP_LAND_STOP,     ACT_LONG_JUMP,      ACT_FREEFALL,      ACT_BEGIN_SLIDING, };
...
void slide_bonk(struct MarioState *m, u32 fastAction, UNUSED u32 slowAction) {
...
void update_sliding_angle(struct MarioState *m, f32 accel, f32 lossFactor) {
    ...
    */
    ...
    if (m->forwardVel > 96.0f) {
        m->slideVelX = m->slideVelX * 96.0f / m->forwardVel;
        m->slideVelZ = m->slideVelZ * 96.0f / m->forwardVel;
    }
    */
...
void update_shell_speed(struct MarioState *m) {
    ...
    if (m->forwardVel <= 0.0f) {
        ...
        f32 vel = 0.75f;
        if (gMovesetVar & DEMO) vel = 0.5f;
        if (gMovesetVar & REAL) vel = 1.0f;
		if (M_TOAD) vel *= 2.0f;
        if (M_LUIGI || m->flags & MARIO_METAL_CAP) vel = 0.5f;
        m->forwardVel += vel;
...
void update_walking_speed(struct MarioState *m) {
    ...
    u32 turnSpd = (!M_LUIGI) ? 24.0f : 18.0f;
...
s32 check_ground_dive_or_punch(struct MarioState *m) {
    ...
    if (m->forwardVel >= 16.0f) {
        if (gMovesetVar & (DEMO | A_MASK)) { (!(gMovesetVar & REAL)) ? (m->vel[1] = 24.0f) : (m->vel[1] = 20.0f); }
...
s32 begin_braking_action(struct MarioState *m) {
    ...
    if (m->forwardVel > 20.0f || (M_LUIGI && !(m->flags & MARIO_METAL_CAP))) {
...
void tilt_body_walking(struct MarioState *m, UNUSED s16 startYaw) {
    ...
    if (animID == MARIO_ANIM_WALKING || animID == MARIO_ANIM_RUNNING) {
        ...
        //! (Speed Crash) These casts can cause a crash if
...
s32 act_turning_around(struct MarioState *m) {
    ...
    if (m->input & INPUT_A_PRESSED && !(gMovesetVar & A_MASK)) {
...
s32 act_decelerating(struct MarioState *m) {
    ...
    if (slopeClass == SURFACE_CLASS_VERY_SLIPPERY) {
        ...
        s32 animSpeed = (s32)(m->forwardVel * 0x4000);
        if (animSpeed < 0x1000) {
            animSpeed = 0x1000;
        }
...
s32 stomach_slide_action(struct MarioState *m, u32 stopAction, u32 airAction, s32 animation) {
    if (m->actionTimer == 5) {
        if (m->input & (INPUT_A_DOWN | INPUT_B_DOWN)) {
#if ENABLE_RUMBLE
...
s32 act_crouch_slide(struct MarioState *m) {
    ...
    if (m->input & INPUT_B_PRESSED) {
        if (m->forwardVel >= 16.0f && !(gMovesetVar & DEMO)) {
            return set_mario_action(m, ACT_SLIDE_KICK, 0);
        } else if (!(gMovesetVar & A_HARD)) {
            if (check_ground_dive_or_punch(m)) return TRUE;
        }
    }

    s32 action = (!(gMovesetVar & DEMO)) ? ACT_LONG_JUMP : ACT_JUMP;
    if (m->input & INPUT_A_PRESSED) {
    ...
    return common_slide_action_with_jump(m, ACT_CROUCHING, action, ACT_FREEFALL,
...
s32 common_ground_knockback_action(struct MarioState *m, s32 animation, s32 checkFrame, s32 playLandingSound, s32 actionArg) {
    ...
    if (actionArg > 0 && m->actionTimer != 4) {
    ...
    if (perform_ground_step(m) == GROUND_STEP_LEFT_GROUND) {
        if (actionArg == 4) return set_mario_action(m, ACT_IDLE, 0); // edge softlock bugfix
        if (m->forwardVel >= 0.0f) {
            m->forwardVel = -0.5f;
        ...
        if (actionArg > 0 && m->actionTimer != 4) m->invincTimer = 30;
        m->forwardVel = 16.0f;
    }
...
s32 common_landing_cancels(struct MarioState *m, struct LandingAction *landingAction,
    ...
    if (++m->actionTimer >= landingAction->numFrames || m->input & INPUT_FIRST_PERSON) {
