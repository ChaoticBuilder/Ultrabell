...
void set_camera_shake_from_hit(s16 shake) {
    switch (shake) {
        ...
        case SHAKE_BURN:
            set_camera_pitch_shake(0x60, 0x3F, 0x8000);
            break;
 ...
 */
s32 update_radial_camera(UNUSED struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    if (!(gCameraMovementFlags & CAM_MOVE_ROTATE)) {
        ...
        if (avoidStatus == 3) {
            ...
            if (c->mode == CAMERA_MODE_RADIAL) {
                ...
                rotateSpeed = sqr(gMarioStates[0].forwardVel / 2.0f);
...
s32 mode_behind_mario(struct Camera *c) {
    ...
    if (distCamToFocus > 0x400) {
        distCamToFocus = 0x400;
 ...
 */
struct CameraTrigger sCamTesting[] = {
	NULL_TRIGGER
};
...
void cutscene_dance_default_rotate(struct Camera *c) {
    ...
    cutscene_event(cutscene_dance_default_focus_mario, c, 0, 80);
...
void cutscene_enter_painting(struct Camera *c) {
    ...
    if (gRipplingPainting != NULL) {
        ...
        focusOffset[0] = 0;
        focusOffset[1] = gRipplingPainting->size / 2;
        focusOffset[2] = 0;
        ...
        approach_vec3f_asymptotic(c->focus, focus, 0, 0.25f, 0);
        focusOffset[2] = -(((gRipplingPainting->size * 1000.f) / 2) / 307.f);
        offset_rotated(focus, paintingPos, focusOffset, paintingAngle);
    
        if (c->cutscene == CUTSCENE_ENTER_PAINTING) {
            approach_vec3f_asymptotic(c->pos, focus, 0, 0.25f, 0);
        } else {
            approach_vec3f_asymptotic(c->pos, focus, 0, 1.0f, 0);
        }
    }
}
...
u8 sZoomOutAreaMasks[] = {
    ZOOMOUT_AREA_MASK(0, 0, 0, 0, 0, 0, 0, 0), // NONE     | NONE
	...
	ZOOMOUT_AREA_MASK(1, 0, 0, 0, 1, 1, 0, 0), // TTM      | THI
	ZOOMOUT_AREA_MASK(0, 0, 0, 0, 1, 0, 0, 0), // TTC      | RR
	ZOOMOUT_AREA_MASK(1, 0, 0, 0, 1, 0, 0, 0), // BITDW    | BITFS
	ZOOMOUT_AREA_MASK(1, 0, 0, 0, 1, 0, 0, 0), // BITS     | TOTWC
	ZOOMOUT_AREA_MASK(1, 0, 0, 0, 0, 0, 0, 0), // VCUTM    | COTMC
	ZOOMOUT_AREA_MASK(1, 0, 0, 0, 1, 0, 0, 0), // PSS      | SA
	ZOOMOUT_AREA_MASK(1, 0, 0, 0, 1, 0, 0, 0), // WMOTR    | BOWSER 1
	ZOOMOUT_AREA_MASK(1, 0, 0, 0, 1, 0, 0, 0), // BOWSER 2 | BOWSER 3
	ZOOMOUT_AREA_MASK(0, 0, 0, 0, 0, 0, 0, 0), // ENDING   | TEST
};
