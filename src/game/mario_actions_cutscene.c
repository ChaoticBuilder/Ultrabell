...
void general_star_dance_handler(struct MarioState *m, s32 isInWater) {
    ...
    if (m->actionState == ACT_STATE_STAR_DANCE_CUTSCENE) {
        switch (++m->actionTimer) {
            case 1:
                if (gStarModelLastCollected == MODEL_BOWSER_KEY) celebStar = spawn_object(m->marioObj, MODEL_STAR, bhvCelebrationKey);
                else                                             celebStar = spawn_object(m->marioObj, MODEL_STAR, bhvCelebrationStar);
...
s32 act_entering_star_door(struct MarioState *m) {
    ...
    f32 targetDZ;
    s16 targetAngle;
    // u8 troll = (random_float() < 0.03125f) ? TRUE : FALSE;
    ...
    if (m->actionTimer++ == 0) {
        ...
        // if (troll) targetAngle += DEGREES(150);
...
s32 act_going_through_door(struct MarioState *m) {
    ...
    u8 troll = (random_u16() < 0x800) ? TRUE : FALSE; /* 0.03125% Chance */
